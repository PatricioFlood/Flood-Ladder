    ...
    <div class="container">
        ...
        <div class="principal">
            ...
            <div class="ladder">
                ...
                <div class="networks">
                    <div class="network" v-for="(i,n) in networks" :key="n">
                        <p>Network {{n+1}}</p>
                        <div class="row" v-for="(i,r) in  network[n].rowsTot" :key="r">
                            <div class="box" v-for="(i,b) in 10" :key="b" @click="select(n,r,b)"
                        ...
                        <div class="aux-row">
                            <div class="aux-box" v-for="(i,ab) in 10" :key="ab"
                            @click="select(n,-1,ab)" 
...
export default {
    ...
    methods: {
        ...
        addNetwork(){
            this.network.push(this.networkModel)
        },

        addRow(network,row=-1){
            var rowsTot = this.network[network].row.length

            this.network[network].rows.push(this.networkModel.row)

            if(row!=-1){
                if(this.selected.r == row)
                    this.network[this.selected.n].row[this.selected.r].box[this.selected.b].selected = false
                    
                var whiteRow = this.network[network].row[rowsTot]
                for(var i = rowsTot; i>row; i--){
                    this.network[network].row.splice(i,1,this.network[network].row[i-1])
                }
                this.network[network].row.splice(row,1,whiteRow)

                for(i = 0; i<this.network[network].row[row].boxs.length; i++){
                    if(this.getClass(network,row+1,i).includes("top"))
                        this.setClass(network,row,i,"simbolo-top simbolo-bottom")
                }
            }
        },
        ...
        deleteSymbol(){
            ...
            const symbol = this.getClass(n,r,b)
            const symbolBefore = b>0?this.getClass(n,r,b-1):""
            ...
            if(/(simbolo-top( )?|simbolo-bottom( )?)+/.test(symbol)){
    ...
    created(){
        ...
        for(var i = 0; i<9; i++){
            this.networkModel.row[0].box.push(this.networkModel.row[0].box[0])
        }
        this.networkModel.row[0].box[0].symbol = "start"
        this.networkModel.row[0].box[0].class = "symbol-start"
        for(i = 0; i<9; i++){
            this.networkModel.auxRow.push(this.networkModel.auxRow[0])
        }
        for(i = 0; i<9; i++){
            this.network.push(this.networkModel)
        }
                            ...
                            .row{
                                ...
                                .box{
                                    ...
                                    .box-background{
                                        width: 100%;
                                        height: 60px;
                                        background-image:   url("/assets/ladder/v-line.jpg"),
                                                            url("/assets/ladder/ladder.png");
                                    ...
                                    .symbol-coil{
                                        --position-y-two:-120px;
                                    }
                                    .sybol-contact{
                                        --position-y-two:-160px;
                                    }
                                    .symbol-continue{
                                        --position-y-two:-200px;
                                    }
                                    .symbol-start{
                                        --position-y-two: -240px;
                                    }
                                    .symbol-line{
                                        --position-y-two: -280px;
                                    }
                                    .symbol-bottom{
